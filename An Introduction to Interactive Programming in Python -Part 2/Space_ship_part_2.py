...
def draw(canvas):
    global time, started, lives, score, missile_group, rock_group
...
#a group_collide helper function
...
def group_group_collide(group_1,group_2):
    num = 0
    for mis in set(group_1):
        if group_collide(group_2,mis):
            num += 1
            group_1.discard(mis)
    return num
        
#a helper function process_sprite_group        
...
def rock_spawner():
    ...
    if started and len(rock_group) < 12:
        ...
        if distance_from_ship > 2*(my_ship.get_radius() + a_rock.get_radius()) :
